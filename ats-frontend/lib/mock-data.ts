import { ApplicationInterface, CandidateInterface, JobInterface, QueueItem } from "./interfaces"
...
export const mockSkills = [
  ...
  export const mockApplications: ApplicationInterface[] = Array.from({ length: 30 }, (_, i) => {
    const id = randomId();
    const candidateId = randomItem(mockCandidates)._id!;
    const jobId = randomItem(mockJobs)._id!;
    const now = new Date();
  ...
  export const mockQueueItems: QueueItem[] = Array.from({ length: 15 }, (_, i) => {
    ...
    switch (entityType) {
      case 'candidate':
        entityId = randomItem(mockCandidates)._id!;
        break;
      case 'job':
        entityId = randomItem(mockJobs)._id!;
        break;
      case 'application':
